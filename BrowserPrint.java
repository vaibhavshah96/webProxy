  	...
  	public static void main(String[] args) throws Exception {
  		...
  		while(true) {
  			...
  			int Length = cli.getInputStream().read(request,0,65535);
  			 ...
  			 parsePort(request, Length);
  		}
  	}

  	private static void parsePort(byte[] request, int Length) throws UnsupportedEncodingException {
  		int port = 80;
  		boolean portGiven = false;
  		byte[] userPort = new byte[100];
  		int z = 0;
  			...
  			if((char)request[p]=='H' && (char)request[p+1]=='o' && (char)request[p+2]=='s' && (char)request[p+3]=='t') {
  				...
  				while((char)request[p]!='\n' ){
  					...
  					if(portGiven) {
  						...
  						port = Integer.parseInt((new String(userPort)).trim());
